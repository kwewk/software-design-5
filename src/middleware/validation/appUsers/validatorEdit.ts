...
export const validatorEditAppUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { name, isRegistered } = req.body;
  const errorsValidation: ErrorValidation[] = [];

  if (name !== undefined && validator.isEmpty(String(name).trim())) {
    errorsValidation.push({ name: 'User name cannot be empty' });
  }
  ...
  if (errorsValidation.length !== 0) {
    const customError = new CustomError(
      400,
      'Validation',
      'Edit app user validation error',
      null,
      null,
      errorsValidation,
    );
